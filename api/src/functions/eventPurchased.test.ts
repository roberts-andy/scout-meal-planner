...
describe('event purchased handler', () => {
  it('returns 401 when no auth context', async () => {
    ...
    const result = await handler(makeReq({ item: 'beans-can', purchased: true }), ctx)
  ...
  it('adds item to purchasedItems when purchased=true', async () => {
    ...
    vi.mocked(cosmos.getById).mockResolvedValueOnce({
      id: 'e1',
      troopId: 'troop-42',
      purchasedItems: ['salt-tsp'],
    } as any)
    vi.mocked(cosmos.update).mockImplementationOnce(async (_c, _id, item) => item as any)

    const result = await handler(makeReq({ item: 'beans-can', purchased: true }), ctx)
    expect(result.jsonBody.purchasedItems).toEqual(['salt-tsp', 'beans-can'])
    expect(cosmos.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        id: 'e1',
        troopId: 'troop-42',
        purchasedItems: ['salt-tsp', 'beans-can'],
  ...
  it('removes item from purchasedItems when purchased=false', async () => {
    ...
    const result = await handler(makeReq({ item: 'beans-can', purchased: false }), ctx)
    expect(result.jsonBody.purchasedItems).toEqual(['salt-tsp'])
  })
})
