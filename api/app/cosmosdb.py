...
async def update_item(container_name: str, item_id: str, item: dict, partition_key_value: str | None = None) -> dict:
    container = _get_container(container_name)
    payload = dict(item)
    etag = payload.pop("_etag", None)
    if etag:
        return await container.replace_item(
            item_id,
            payload,
            partition_key=partition_key_value or item_id,
            etag=etag,
            match_condition=MatchConditions.IfNotModified,
        )
    return await container.replace_item(item_id, payload, partition_key=partition_key_value or item_id)
